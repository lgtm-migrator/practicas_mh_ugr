...
if sys.version_info < (3, ):
    ...
    class LineIterator(BaseLineIterator):
        ...
        def __next__(self):
            lineno, line = next(super(LineIterator, self))
            if self._encoding:
...
class ProfileBase(object):
    ...
    def callgrind(self, out, filename=None, commandline=None, relative_path=False):
        ...
        """
        print('# callgrind format', file=out)
        print('version: 1', file=out)
        print('creator: pprofile', file=out)
        print('event: usphit :microseconds/hit', file=out)
        print('events: hits microseconds usphit', file=out)
        ...
        for current_file in self._getFileNameList(filename, may_sort=False):
            ...
            print('fl=%s' % convertPath(current_file), file=out)
            ...
            for (
                ...
                append('cfl=' + convertPath(callee_file))
                append('cfn=' + getCodeName(callee_file, callee))
                append('calls=%i %i' % (call_hits, callee.co_firstlineno))
                append('%i %i %i %i' % (lineno, call_hits, call_ticks, call_ticks // call_hits))
            for func_name, line_dict in func_dict.items():
                print('fn=%s' % func_name, file=out)
            ...
            for lineno, line in LineIterator(
                ...
                for (
                    ...
                    print(_ANNOTATE_CALL_FORMAT % {
                        'hits': call_hits,
                        'time': call_duration,
                        'time_per_hit': call_duration / call_hits,
                        'percent': percent(call_duration, total_time),
                        'callee_file': callee_file,
                        'callee_line': callee.co_firstlineno,
                        'callee_name': callee.co_name,
                    }, file=out)
...
class Profile(ProfileBase, ProfileRunnerBase):
    ...
    def enable(self):
        ...
        if self.enabled_start:
            ...
            self._enable()
            sys_settrace(self._global_trace)
...
class ThreadProfile(Profile):
    ...
    def _enable(self):
        ...
        threading_settrace(self._global_trace)
